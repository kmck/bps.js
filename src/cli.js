...
import applyBpsPatch from './applyBpsPatch';
import createBpsPatch from './createBpsPatch';

function applyBpsPatchFile(source, patch, target, options = {}) {
  const {
  ...
  try {
    targetFile = applyBpsPatch(sourceFile, patchFile, {
      log(...args) {
        if (dryRun) {
          logger.info(...args);
        } else {
...
function createBpsPatchFile(source, target, patch, options = {}) {
  const {
    allowOverwrite = false,
    dryRun = false,
  } = options;
  ...
  try {
    patchFile = createBpsPatch(sourceFile, targetFile, {
      log(...args) {
        if (dryRun) {
          logger.info(...args);
        } else {
...
program
  ...
  .action((source, patch, target, cmd) => {
    ...
    const result = applyBpsPatchFile(source, patch, target, {
...
program
  ...
  .action((source, target, patch, cmd) => {
    if (!patch) {
      ...
      // eslint-disable-next-line
      patch = path.join(dir, `${name}.bps`);
    }
    const result = createBpsPatchFile(source, target, patch, {
      allowOverwrite: cmd.allowOverwrite,
      dryRun: cmd.dryRun,
    });
    logger.on('finish', () => {
